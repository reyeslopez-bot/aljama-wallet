...
describe('app/api/xrpl/nft/offer/cancel route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('requires at least one offer id', async () => {
    const { POST } = await import('@/app/api/xrpl/nft/offer/cancel/route')
    const res = await POST(new Request('http://localhost/api/xrpl/nft/offer/cancel', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ idempotencyKey: '11111111-1111-4111-8111-111111111111', offerIds: [] }),
    }))

    expect(res.status).toBe(400)
  })

  it('submits nft offer cancel transaction', async () => {
    const { POST } = await import('@/app/api/xrpl/nft/offer/cancel/route')
    const res = await POST(new Request('http://localhost/api/xrpl/nft/offer/cancel', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ idempotencyKey: '11111111-1111-4111-8111-111111111111', offerIds: ['000000000000000001'] }),
    }))
