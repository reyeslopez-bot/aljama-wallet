...
describe('app/api/xrpl/nft/offer/create route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rejects invalid destination address', async () => {
    const { POST } = await import('@/app/api/xrpl/nft/offer/create/route')
    const res = await POST(new Request('http://localhost/api/xrpl/nft/offer/create', {
      method: 'POST',
  ...
  it('submits nft offer create transaction', async () => {
    const { POST } = await import('@/app/api/xrpl/nft/offer/create/route')
    const res = await POST(new Request('http://localhost/api/xrpl/nft/offer/create', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        idempotencyKey: '11111111-1111-4111-8111-111111111111',
        nftokenId: '0000000000000000ABCDEFABCDEFABCDEF',
        mode: 'sell',
        amountXrp: '2',
      }),
    }))
