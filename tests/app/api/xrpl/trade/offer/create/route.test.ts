...
vi.mock('@/lib/xrpl-amount', () => ({ toXrplAmount: vi.fn((v) => v) }))

describe('app/api/xrpl/trade/offer/create route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when session missing', async () => {
    ...
    const { POST } = await import('@/app/api/xrpl/trade/offer/create/route')
    const res = await POST(new Request('http://localhost/api/xrpl/trade/offer/create', {
      method: 'POST',
  ...
  it('submits offer create transaction', async () => {
    const { POST } = await import('@/app/api/xrpl/trade/offer/create/route')
    const res = await POST(new Request('http://localhost/api/xrpl/trade/offer/create', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        ...
        takerGets: { currency: 'USD', issuer: 'rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe', value: '10' },
        takerPays: { currency: 'XRP', value: '20' },
      }),
    }))
