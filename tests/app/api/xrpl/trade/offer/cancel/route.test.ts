...
describe('app/api/xrpl/trade/offer/cancel route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('validates offerSequence', async () => {
    const { POST } = await import('@/app/api/xrpl/trade/offer/cancel/route')
    const res = await POST(new Request('http://localhost/api/xrpl/trade/offer/cancel', {
      method: 'POST',
  ...
  it('submits offer cancel transaction', async () => {
    const { POST } = await import('@/app/api/xrpl/trade/offer/cancel/route')
    const res = await POST(new Request('http://localhost/api/xrpl/trade/offer/cancel', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ idempotencyKey: '11111111-1111-4111-8111-111111111111', offerSequence: 7 }),
    }))
